class AddCustomEtablissementJuly < ActiveRecord::Migration[5.0]
  def change
    create_table :custom_etablissement_julies do |t|
    ...
    execute "
      create index on custom_etablissement_julies using gin(to_tsvector('french', siren));
      create index on custom_etablissement_julies using gin(to_tsvector('french', siret));
      create index on custom_etablissement_julies using gin(to_tsvector('french', activite_principale));
      create index on custom_etablissement_julies using gin(to_tsvector('french', l6_normalisee));
      create index on custom_etablissement_julies using gin(to_tsvector('french', nom_raison_sociale));"
  end
end
